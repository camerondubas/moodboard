...
pub fn App(
    ...
    let (shared, set_shared) = create_signal(shared.clone());
    let name = shared.get().lock().unwrap().name.clone();

    view! {
        <div class="static ">
            <Moodboard plugin={plugin} shared={shared} />
            ...
            <ControlPanel events={events.clone()} shared={shared} set_shared={set_shared} />
        </div>
    }
}
